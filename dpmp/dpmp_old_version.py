    ...
    async def pool_reader(self, pool_key: str, reader: asyncio.StreamReader):
        ...
        async for raw in iter_lines(reader, side):
            ...
            if method == "mining.notify":
                ...
                log("pool_notify", pool=pool_key, jobid=jid, seq=self.notify_seq[pool_key])
                continue
            ...
            if "id" in msg and msg.get("method") is None:
                ...
                if mid in self.submit_owner:
                    ...
                    if ok:
                        ...
                        log("share_result", pool=p, accepted=True)
                    else:
                        SHARES_REJECTED.labels(pool=p).inc()
                        log("share_result", pool=p, accepted=False, error=msg.get("error"))
    ...
    async def forward_jobs(self):
        ...
        while True:
            ...
            if pick != current_pool and (now - last_switch_ts) < min_switch:
                # Try to stay on current pool if it has something new
                if self.notify_seq[current_pool] > last_seen[current_pool]:
                    pick = current_pool
            ...
            log("job_forwarded", pool=pick, jobid=jid, seq=last_seen[pick])
